          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                <p className="text-sm text-muted-foreground">좋아하는 음식 종류를 선택해주세요 (여러 개 선택 가능)</p>
                ...
                {formData.preferences.length > 0 && (
                  <div className="mt-4">
                    <p className="text-sm font-medium mb-2">선택된 선호 음식:</p>
                    <div className="flex flex-wrap gap-2">
                      {formData.preferences.map((preference) => (
                        <Badge key={preference} variant="secondary" className="flex items-center gap-1">
                          {preference}
                          <X className="w-3 h-3 cursor-pointer" onClick={() => handlePreferenceToggle(preference)} />
                        </Badge>
                      ))}
                    </div>
                  </div>
